...
var _SCORE_BG_GRADIENT = {
  baseColor: '#02020cff',
  horizontalGradient: {
    left: 'rgba(0, 208, 255, 0.36)',
    middle: 'rgba(44, 0, 80, 0.16)',
    right: 'rgba(21, 0, 34, 0.1)'
  },
  verticalGradient: {
    top: 'rgba(0, 208, 255, 0.14)',
    middle1: 'rgba(121, 0, 219, 0.05)',
    middle2: 'rgba(69, 4, 223, 0.05)',
    bottom: 'rgba(212, 0, 255, 0.08)'
  },
  radialGlow: {
    center: 'rgba(0, 0, 0, 0)',
    middle: 'rgba(0, 0, 0, 0)',
  ...
  edgeLines: {
    top: 'rgba(0, 230, 255, 0.5)',
    bottom: 'rgba(162, 0, 255, 0.49)'
  }
};
...
var _NOTE_GRADIENTS = {
  'C': ['hsla(14, 79%, 37%, 1.00)', 'hsla(17, 62%, 39%, 1.00)', 'hsla(0, 66%, 43%, 1.00)', 'hsla(0, 66%, 49%, 1.00)', 'hsla(350, 79%, 50%, 1.00)'],
  'D': ['hsla(26, 57%, 37%, 1.00)', 'hsla(22, 45%, 41%, 1.00)', 'hsla(24, 57%, 46%, 1.00)', 'hsla(24, 77%, 44%, 1.00)', 'hsla(34, 100%, 45%, 1.00)'],
  'E': ['hsla(44, 32%, 47%, 1.00)', 'hsla(48, 37%, 50%, 1.00)', 'hsla(49, 51%, 48%, 1.00)', 'hsla(59, 59%, 49%, 1.00)', 'hsla(56, 85%, 51%, 1.00)'],
  'F': ['hsla(82, 40%, 41%, 1.00)', 'hsla(86, 46%, 43%, 1.00)', 'hsla(101, 49%, 41%, 1.00)', 'hsla(106, 63%, 40%, 1.00)', 'hsla(129, 98%, 43%, 1.00)'],
  'G': ['hsla(208, 38%, 48%, 1.00)', 'hsla(199, 41%, 52%, 1.00)', 'hsla(199, 52%, 43%, 1.00)', 'hsla(189, 64%, 42%, 1.00)','hsla(182, 93%, 43%, 1.00)'],
  'A': ['hsla(231, 45%, 42%, 1.00)', 'hsla(231, 46%, 41%, 1.00)', 'hsla(216, 58%, 36%, 1.00)', 'hsla(216, 67%, 36%, 1.00)','hsla(214, 97%, 40%, 1.00)'],
  'B': ['hsla(263, 55%, 46%, 1.00)', 'hsla(271, 49%, 43%, 1.00)', 'hsla(274, 65%, 42%, 1.00)', 'hsla(285, 76%, 40%, 1.00)','hsla(295, 92%, 40%, 1.00)']
};
...
function _recLoop() {
  ...
  _recPlayState.raf = requestAnimationFrame(function(now) {
    ...
    // Scroll - 视觉速度不受滚动栏缩放影响
    var scoreW = scoreWrap ? scoreWrap.clientWidth : 300;
    var fixedX = scoreW / 3;
    var currentH = scoreWrap ? scoreWrap.offsetHeight : 170;
    var scale = currentH / (_recPlayState.baseLaneH * 5);
    // 视觉滚动偏移：不受 scale 影响，保持固定的屏幕速度
    var nowX = elapsed * 0.18 * _VISUAL_SPEED_MULT;
  ...
  if (isUserPlay && typeof _recPlayState !== "undefined" && _recPlayState &&
    ...
    if (fp < fn.length) {
      ...
      if (expected.note === ns) {
        ...
        var nowX2 = _recPlayState.pausedElapsed * 0.18 * _VISUAL_SPEED_MULT;
...
window.onload = function(){
  ...
  function exitLayoutMode(doSave) {
    ...
    // Restore scroll positions after renderPiano completes (must be after 300ms default scroll)
    if (doSave) {
      setTimeout(() => {
        container.querySelectorAll('.octave-row').forEach((row, i) => {
          const kw = row.querySelector('.octave-row-keys');
  ...
  function loadLayoutSettings() {
    try {
      ...
      if (saved) {
        ...
        // Use 400ms to run AFTER renderPiano's 300ms default scroll timer
...
function _applyFullPos(full){
 if(!_fullBtnRect)_snapAutoBtn();
 if(!_fullBtnRect)return;
 full.style.left=_fullBtnRect.left+"px";
 full.style.top=_fullBtnRect.top+"px";
 full.style.width=_fullBtnRect.width+"px";
 full.style.height=_fullBtnRect.height+"px";
}
function updateFullscreenBtn(h){
 ...
 if(shouldShow){
   ...
   if(!full){
     ...
     full.style.cssText="position:fixed;border-radius:8px;border:1px solid #444;background:#222;color:#0099ff;font-size:15px;cursor:pointer;z-index:9999;display:flex;align-items:center;justify-content:center;padding:0;touch-action:manipulation;";
...
function toggleScoreFullscreen(){
 ...
 if(isScoreFullscreen){
   ...
   musicBar.style.zIndex = '9998';
   musicBar.style.background = 'rgba(0,0,0,0.95)';
   
   const control = document.querySelector('.control');
   if (document.body.classList.contains('toolbar-rotated-90')) {
     musicBar.style.left = '0';
     musicBar.style.right = '0';
     if (control) control.style.display = 'none';
   } else if (document.body.classList.contains('toolbar-rotated-270')) {
     musicBar.style.left = '0';
     musicBar.style.right = '0';
     if (control) control.style.display = 'none';
   } else {
